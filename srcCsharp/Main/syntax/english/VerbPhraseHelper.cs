...
namespace SimpleNLG.Main.syntax.english
{
     ...
     * @version 4.0
     */
	internal abstract class VerbPhraseHelper
	{
	    /**
		...
		private static void splitVerbGroup(Stack<NLGElement> vgComponents, Stack<NLGElement> mainVerbRealisation, Stack<NLGElement> auxiliaryRealisation)
		{
			...
			bool mainVerbSeen = false;

            foreach (NLGElement word in vgComponents.Reverse()) //Note: Reverse to ensure same order of processing as java
			{
				if (!mainVerbSeen)
				{
	     ...
	     */
		private static Stack<NLGElement> createVerbGroup(SyntaxProcessor parent, PhraseElement phrase)
		{
			...
			bool modalPast = false;
			Stack<NLGElement> vgComponents = new Stack<NLGElement>();
			bool interrogative = phrase.hasFeature(Feature.INTERROGATIVE_TYPE);
			...
			{
				if (Tense.FUTURE.Equals(tenseValue) && ReferenceEquals(modal, null) && ((!(phrase.getHead() is CoordinatedPhraseElement)) || (phrase.getHead() is CoordinatedPhraseElement && interrogative)))
				{

					actualModal = "will"; //$NON-NLS-1$

				}
				else if (!ReferenceEquals(modal, null))
				{
					actualModal = modal;
			...
			NLGElement frontVG = grabHeadVerb(phrase, tenseValue, !ReferenceEquals(modal, null));
			...
			frontVG = pushIfModal(!ReferenceEquals(actualModal, null), phrase, frontVG, vgComponents);
			frontVG = createNot(phrase, vgComponents, frontVG, !ReferenceEquals(modal, null));
		...
		{
			if (!ReferenceEquals(actualModal, null) && !phrase.getFeatureAsBoolean(InternalFeature.IGNORE_MODAL))
			{
				vgComponents.Push(new InflectedWordElement(actualModal, new LexicalCategory(LexicalCategory.LexicalCategoryEnum.MODAL)));
			}
		}
